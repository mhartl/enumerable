...
class TestRubyDevSummit < Minitest::Test
  ...
  def test_return_first_n_elements
    assert_equal [1, 2, 3], @rubydevsummit.return_first_n_elements(@enum, 3)
  end
  ...
  def test_product
    assert_equal 120, @rubydevsummit.product(@enum)
  end
  ...
  def test_days_of_the_week
    out = capture_stdout do
      @rubydevsummit.days_of_the_week
    end
  ...
  def test_first_n_primes
    assert_equal [2, 3, 5, 7, 11, 13, 17, 19, 23], @rubydevsummit.first_n_primes(9)
  end
end
